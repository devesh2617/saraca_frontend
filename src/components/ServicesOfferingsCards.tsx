...
const Card: FC<CardProps> = ({ cardsData, imagePosition, imageSrc }) => {
  ...
  return (
    <div
      className={`flex flex-col justify-between container mx-auto bg-white px-8 rounded-3xl md:flex-row ${imagePosition === "right" ? "md:flex-row-reverse" : ""} sm:min-h-24 hover:-translate-y-2 hover:shadow-sm transition-all duration-500` }
    >
      ...
      <div aria-label="content" className={`text-left flex py-12 px-8 md:text-left`}>
        <div className={`${imagePosition==="left"?"md:pl-8":""}`}>
