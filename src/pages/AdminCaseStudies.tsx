...
const AdminWebinar = () => {
    ...
    const handleDelete = async(id) => {
        try {
           const res:any = await getApi(`admin/delete_case_study/${id}`)
           if(res?.data?.message) {
    ...
    useEffect(() => {
        if(firstTime){
        getApi(`api/get_case_studies`).then((res:any) => {
            setData(res?.data?.caseStudies)
            setCurrentData(res.data.caseStudies.slice(0,itemsPerPage))
            setFirstTime(false)
        })}
                ...
                <TableBody>
                    {currentData?.map((item) => (
                        <TableRow key={item?.id}>
                            ...
                            <TableCell onClick={() => { navigate(`/${import.meta.env.VITE_ADMIN_ROUTES_STRING}/admin/edit_case_study/${item.id}`) }}><Button variant="ghost"><Pencil /></Button></TableCell>
