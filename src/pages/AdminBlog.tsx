...
const AdminBlog = () => {
    ...
    const handleDelete = async(id) => {
        try {
           const res:any = await getApi(`admin/delete_blog/${id}`)
           if(res?.data?.message) {
    ...
    useEffect(() => {
        if(firstTime){
        getApi(`api/get_blogs`).then((res:any) => {
            setData(res?.data?.blogs)
            setCurrentData(res.data.blogs.slice(0,itemsPerPage))
            setFirstTime(false)
        })}
                ...
                <TableBody>
                    {currentData?.map((item) => (
                        <TableRow key={item?.id}>
                            ...
                            <TableCell onClick={() => { navigate(`/${import.meta.env.VITE_ADMIN_ROUTES_STRING}/admin/edit_blog/${item.id}`) }}><Button variant="ghost"><Pencil /></Button></TableCell>
