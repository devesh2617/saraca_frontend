...
const AdminWebinar = () => {
    ...
    const handleDelete = async(id) => {
        try {
           const res:any = await getApi(`admin/delete_webinar/${id}`)
           if(res?.data?.message) {
    ...
    useEffect(() => {
        if(firstTime){
        getApi(`api/get_webinars`).then((res:any) => {
            setData(res?.data?.webinars)
            setCurrentData(res.data.webinars.slice(0,itemsPerPage))
            setFirstTime(false)
        })}
                ...
                <TableBody>
                    {currentData?.map((item) => (
                        <TableRow key={item?.id}>
                            ...
                            <TableCell onClick={() => { navigate(`/${import.meta.env.VITE_ADMIN_ROUTES_STRING}/admin/edit_webinar/${item.id}`) }}><Button variant="ghost"><Pencil /></Button></TableCell>
